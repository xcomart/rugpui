//! The application-level shell three `rugpui` applications were each carrying a
//! copy of.
//!
//! `rugpui` is the widget kit: buttons, inputs, trees, grids, the theme layer they
...
//! rugpui_shell::set_update_policy(Box::new(Settings), cx);
//! ```
//!
...
//! | [`catalog`] | [`ThemeCatalog`], and the two implementations over `rugpui`'s own palette formats. |
...
//!   `if let Some(path) = rugpui_shell::restart_path() { cx.set_restart_path(path); }`
