...
pub(crate) fn current_identity() -> AppIdentity {
    try_current_identity().expect("rugpui_shell::init has to run before the update paths do")
}
...
///
/// [`rugpui::TextInput`] draws no edit menu until it is told what to call the
...
/// function — `TextInput::new(cx).context_menu(rugpui_shell::input_menu_labels)`
