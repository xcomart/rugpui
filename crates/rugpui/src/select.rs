...
impl Select {
    ...
    /// list is open or closed: a select's chevron always points down, unlike a
...
mod tests {
    ...
    /// [`Select::chevron_icon`] swaps [`CARET_DOWN`] for an `svg` at the given
    /// path. The test `AssetSource` answers every path with `None`, which
    /// gpui draws as nothing — the default included — so this only proves the
    /// swap does not panic layout or paint.
    #[gpui::test]
