...
//!
//! Two implementations ship here — [`UiThemeCatalog`] over [`ruui::ThemeFile`]
//! and [`EditorThemeCatalog`] over [`ruui::EditorThemeFile`] — and a host with
//! a third kind of palette of its own writes one in its own crate, over
//! [`CatalogFile::Other`].
//!
//! # Why the refusals are a type
//!
//! [`ThemeCatalog::read`] is where this crate is *stricter* than `ruui`'s
...
///
/// The two `ruui` formats are named because this crate's own two catalogues
...
///
/// Stricter than [`ruui::parse_hex`] on purpose: that helper takes an alpha
...
pub fn valid_hex(value: &str, alpha: bool, optional: bool) -> bool {
    ...
    length_ok && ruui::parse_hex(trimmed).is_some()
}
...
mod tests {
    ...
    use ruui::{EditorTheme, Theme, to_hex};
