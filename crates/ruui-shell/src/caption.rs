...
//! costs nothing here — an application has one theme at a time, for the one
//! window it owns. It matters under the custom title bar too, where AppKit still draws
//! the traffic lights over the toolbar band.
//!
//! Nothing on Linux: the caption there is the compositor's, themed by the
//! desktop rather than by the window, and when the application draws its own
//! it draws it out of the same palette as the rest of the chrome.
//!
//! [`TitlebarStyle::System`]: crate::TitlebarStyle
