//! How an area is divided into panes, and what one pane's tab strip does.
//!
//! An area shows a binary tree: every leaf is one pane holding one payload, and
...
//!
//! [`PaneTree`] is deliberately free of gpui types: the promotion and collapse
...
//!
//! [`Pane`] is the payload an application whose panes hold *tabs* reaches for:
//! a list of items, which of them is on top, and the strip's own scroll
//! position. It is generic over the item for the same reason the tree is
//! generic over the payload — what a tab *is* differs per application and none
//! of the strip's rules depend on it — so an application instantiates
//! `PaneTree<Pane<MyTab>>` and adds whatever lookups its own tabs need
//! (`the Generate tab`, `the tab editing this file`) as an extension trait or
//! as free functions over [`Pane::items`].
//!
//! Nothing here is serialised: a layout is what the user has arranged in this
//! session, and none of the applications using this crate store one.
//!
...
/// One pane of a tab-holding area: its tabs, and which of them is on top.
///
...
/// last drawn frame — and why the applications using this reclaim the keyboard
/// around every call that changes what [`Pane::active`] returns.
#[derive(Debug)]
pub struct Pane<I> {
...
// Derived rather than written out, except that `#[derive(Default)]` would
// demand `I: Default` for a field that is only ever an empty `Vec`.
impl<I> Default for Pane<I> {
    fn default() -> Self {
        Self {
            items: Vec::new(),
            active: 0,
...
impl<I> Pane<I> {
    ...
    pub fn active(&self) -> Option<&I> {
        self.items.get(self.active)
    }

    /// The tab on top, to be changed in place.
    pub fn active_mut(&mut self) -> Option<&mut I> {
    ...
    ///
    /// What a dirty marker travels through: the buffer is the truth and the tab
    /// is a copy of one flag off it, refreshed whenever the buffer says it
    /// changed.
    ...
    pub fn close(&mut self, index: usize) -> Option<I> {
    ...
    /// The index of the first tab `matches` accepts.
    ///
    /// The one lookup every application's own — "the tab editing this file",
    /// "the one Generate tab" — is written in terms of, so that a pane does not
    /// have to hand its list out to be searched.
    pub fn position(&self, matches: impl FnMut(&I) -> bool) -> Option<usize> {
...
mod tests {
    ...
    /// focus dance: what `push` brings to the top, where `close` leaves the
    /// marker, and that an empty pane answers every question without panicking
    /// — the state a freshly split pane starts in and the one it returns to
    /// when its last tab is closed.
    #[test]
    fn an_empty_pane_has_no_active_tab_and_nothing_to_close() {
        let mut pane: Pane<&str> = Pane::new();
        assert!(pane.is_empty());
        assert!(pane.active().is_none());
        assert!(pane.active_mut().is_none());
    ...
    fn a_new_tab_comes_to_the_top_and_closing_walks_leftwards() {
        ...
        assert_eq!(pane.items(), ["generate", "a.java", "b.java"]);

        // The tab after the closed one takes its place...
        assert!(pane.activate(1));
        assert_eq!(pane.close(1), Some("a.java"));
        assert_eq!(pane.active_index(), 1);
        assert_eq!(pane.active(), Some(&"b.java"));

        // ...and the one before it when nothing follows.
        assert_eq!(pane.close(1), Some("b.java"));
        assert_eq!(pane.active_index(), 0);
        assert_eq!(pane.active(), Some(&"generate"));
    }

    #[test]
    fn closing_ahead_of_the_active_tab_keeps_the_same_tab_on_top() {
        let mut pane = Pane::new();
        pane.push(1u32);
        pane.push(2);
        pane.push(3);
        // `push` already brought it to the top; the point of the test is what
        // closing an *earlier* tab does to the marker.
        assert_eq!(pane.active_index(), 2);

        assert_eq!(pane.close(0), Some(1));
        assert_eq!(pane.active_index(), 1);
        assert_eq!(pane.active(), Some(&3));
    }

    #[test]
    fn a_pane_finds_a_tab_by_whatever_the_application_recognises_it_by() {
        let mut pane = Pane::new();
        pane.push("model.java");
        pane.push("generate");
        pane.push("mapper.xml");

        assert_eq!(pane.position(|item| *item == "generate"), Some(1));
        assert_eq!(pane.position(|item| item.ends_with(".xml")), Some(2));
        assert_eq!(pane.position(|item| *item == "php.php"), None);
    }

    #[test]
    fn a_tab_can_be_changed_where_it_stands() {
        let mut pane = Pane::new();
        pane.push(String::from("clean"));
        *pane.get_mut(0).expect("one tab") = String::from("dirty");
        assert_eq!(pane.get(0).map(String::as_str), Some("dirty"));
        assert_eq!(pane.get(1), None);

        pane.active_mut().expect("one tab").push('!');
        assert_eq!(pane.active().map(String::as_str), Some("dirty!"));
    }
}
