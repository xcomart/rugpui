...
fn a_pushed_palette_is_what_the_editor_draws_in(cx: &mut TestAppContext) {
    let (editor, mut cx) = open("select 1;", cx);
    cx.update(|_, cx| rugpui::set_editor_theme(EditorTheme::one_light(), cx));
    draw(&mut cx);
    assert_eq!(
    ...
    cx.update(|_, cx| rugpui::set_editor_theme(EditorTheme::gruvbox_dark(), cx));
    draw(&mut cx);
    assert_eq!(
        cx.update(|_, cx| editor.editor.read(cx).palette(cx)),
        EditorTheme::dracula()
